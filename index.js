...
//         };
//         city_name.textContent = data.name
//         const today = new Date(data.dt * 1000);  // dt is current timestamp from API
//         const dayOptions = {
//             weekday: 'long',
//             year: 'numeric',
//             month: 'long',
...
//         let [uvData, aqiData] = await Promise.all([uvindex(lat, lon), aqi(lat, lon)])
//         uv_index.textContent = Math.round(uvData.value)
//         air_quality.textContent = aqiData.list[0].main.aqi
//         // let lat = data.coord.lat;
//         // let lon = data.coord.lon;
...
//         // Initialize map once
//         if (!cityMap) {
//             cityMap = L.map('city-map').setView([lat, lon], 10);  // zoom 10 looks good
...
//         // If marker exists, remove/update it
//         if (cityMarker) {
//             cityMarker.setLatLng([lat, lon]);
//         } else {
//             cityMarker = L.marker([lat, lon]).addTo(cityMap);
...
//         let forecastdata = await fetchForecast(searchcity)
//         let fiveDayForecast = forecastdata.filter((item, index) => index % 8 === 4)
//         let fiveDays = fiveDayForecast.slice(0, 5)
//         forecastContainer.innerHTML = ""
//         fiveDays.forEach(item => {
...
//             let iconcode = item.weather[0].icon
//             let iconUrl = `https://openweathermap.org/img/wn/${iconcode}.png`
//             card.innerHTML = `<div class="day-name">${dayName}</div>
// <img src="${iconUrl}" alt="${item.weather[0].description}">
// <div class="temp-range">${Math.round(item.main.temp)}°C</div>`
//             forecastContainer.append(card)
//         });
